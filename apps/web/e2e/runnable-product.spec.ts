...
test('reload resumes a partial mobile questionnaire and rebuilds completed results', async ({ page }) => {
  ...
  const primaryName = await page.locator('.rl-primary-grid .rl-result-card h3').first().textContent()
  await page.reload()
  await expect(page).toHaveURL(/\/results\/?$/)
  await expect(page.locator('.rl-primary-grid .rl-result-card h3').first()).toHaveText(primaryName ?? '')
