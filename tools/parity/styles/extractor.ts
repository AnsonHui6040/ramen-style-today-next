...
import {
  ...
  styleExtractorVersion,
  styleFixtureManifestSchema,
  styleFixtureSchemaVersion,
  styleInstrumentationVersion,
...
export const styleExtractorAuthoringSourcePaths = [
...
export const legacyStyleSourceIdentity = {
  repository: {
...
export const styleTrackedSourceHashes = {
...
export const styleInstrumentationDescriptor: InstrumentationTransactionDescriptor = Object.freeze({
  targets: Object.freeze([{
    path: 'src/parity-style-observer.test.ts',
    status: '??' as const,
  }]),
...
export const styleExpectedLineage: CopyValidatedExpectedExtractorLineage = Object.freeze({
  identity: Object.freeze({ ...legacyStyleSourceIdentity.repository }),
  commit: legacyStyleSourceIdentity.commit,
  treeHash: legacyStyleSourceIdentity.treeHash,
  trackedSourceHashes: Object.freeze({ ...styleTrackedSourceHashes }),
  lockfilePath: 'package-lock.json',
  lockfileHash: styleLegacyLockfileHash,
  patchHash: styleInstrumentationHash,
  seedsHash: styleSeedsHash,
  nodeVersion: styleExtractionNodeVersion,
})

export interface StyleAuthoringSource extends SharedAuthoringSource {
  readonly relativePath: (typeof styleExtractorAuthoringSourcePaths)[number]
}

export interface StyleExtractorEnvironment extends Omit<
  AuthoringEnvironment<CopyValidatedExpectedExtractorLineage>,
  'authoringSources'
> {
  readonly authoringSources: readonly StyleAuthoringSource[]
}

export interface CreateStyleExtractorEnvironmentInput extends Omit<
...
export type StyleExtractorResult = FixtureAuthoringResult<
  LegacyStyleObservation,
  StyleFixtureManifest
>
export type StyleExtractorCommandResult = FixtureAuthoringCommandResult<
  LegacyStyleObservation,
  StyleFixtureManifest
>

const toolRoot = fileURLToPath(new URL('../../../', import.meta.url))
const defaultAuthoringSources = styleExtractorAuthoringSourcePaths.map((relativePath) => ({
  relativePath,
  path: resolve(toolRoot, relativePath),
}))

function buildStyleFixtureManifest(
  input: ManifestBuildInput<
    LegacyStyleObservation,
    CopyValidatedExpectedExtractorLineage
  >,
) {
  const observation = input.cases[0]
  if (!observation || input.cases.length !== 1) {
    throw new Error('style manifest requires one complete observation')
  }
  if (input.dependencyProvisioning.kind !== 'copy-validated') {
    throw new Error('style manifest requires copy-validated dependencies')
  }
  return styleFixtureManifestSchema.parse({
    fixtureSchemaVersion: styleFixtureSchemaVersion,
    source: {
    ...
    extractor: {
      version: styleExtractorVersion,
      sources: input.authoringSources,
      hash: computeStyleExtractorAuthoringHash(input.authoringSources),
    },
    instrumentation: {
      version: styleInstrumentationVersion,
    ...
    orderedAdjustmentCopyIds: orderedAdjustmentCopyIds(observation.adjustments),
    orderedExclusionTagIds: observation.exclusionTags.map(({ id }) => id),
    orderedCopyRoles: observation.copyRoles.map(({ id }) => id),
    coverage: observation.coverage,
    caseCount: 1,
    casesHash: computeLegacyStyleCasesHash(input.cases),
    fixtureContentHash: input.fixtureContentHash,
  })
}

export const styleAuthoringAdapter: FixtureAuthoringAdapter<
  LegacyStyleSeedCase,
  LegacyStyleObservation,
  StyleFixtureManifest,
  CopyValidatedExpectedExtractorLineage
> = {
  parseSeeds: (input) => [legacyStyleSeedFileSchema.parse(input).case],
  parseRawCases: parseLegacyStyleRawCases,
  validateCases: validateLegacyStyleCases,
  buildManifest: buildStyleFixtureManifest,
  serializeCases: serializeLegacyStyleCases,
  serializeManifest: serializeStyleFixtureManifest,
}

export function createStyleExtractorEnvironment(
  input: CreateStyleExtractorEnvironmentInput,
): StyleExtractorEnvironment {
  const authoringSources = input.authoringSources ?? defaultAuthoringSources
  if (
    authoringSources.length !== styleExtractorAuthoringSourcePaths.length
    || authoringSources.some((source, index) => (
      source.relativePath !== styleExtractorAuthoringSourcePaths[index]
    ))
  ) throw new Error('style authoring source set mismatch')
  return createAuthoringEnvironment({
    ...input,
    authoringSources,
    expected: styleExpectedLineage,
    instrumentation: styleInstrumentationDescriptor,
  }) as StyleExtractorEnvironment
}

export function runStyleExtractor(
  environment: StyleExtractorEnvironment,
  options: RunFixtureAuthoringOptions,
): Promise<StyleExtractorResult> {
  return runFixtureAuthoring(
    environment,
    styleAuthoringAdapter,
    options,
  )
}

export function runStyleExtractorCommand(
  environment: StyleExtractorEnvironment,
  options: RunFixtureAuthoringOptions,
