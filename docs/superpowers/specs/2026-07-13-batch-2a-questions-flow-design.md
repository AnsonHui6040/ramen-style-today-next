...
For maximum-selection observation, instrumentation reads the disabled state actually exposed by the rendered legacy component or DOM. `disabledOptionIds` contains only currently visible disabled options, in legacy display order; hidden options are forbidden. Selected ordinary options are not classified as disabled merely because the cap is reached unless the legacy UI directly exposes them that way, and exclusive-option disabled state is observed rather than inferred. Instrumentation must not invoke a disabled option's click handler or synthesize an option action. The authoring seed reaches the cap through real public selections and records the disabled third ordinary option in the settled frame; it contains no extra `select` action for that disabled option.
...
Trace coverage tags are limited to facts mechanically derivable from validated actions and observable frames: displayed question IDs, visible and disabled option IDs scoped to their question, action types, transition types, forced-skip, navigation targets, completion, validated pending-selection effects, `behavior:max-selection-blocked`, and observed branch changes. `behavior:max-selection-blocked` requires a settled frame that directly observes a visible unselected ordinary option as disabled after real public selections reached the cap; it never requires or accepts a click action for that disabled option. `behavior:max-no-op` is removed from observable parity. Seeds never contain coverage tags; visible/disabled options, navigation targets, forced transitions, completion, and named behavior outcomes therefore cannot become hand-authored seed expectations. Semantic-class, repair, diagnostic, invalid-input, dependency, reachability, and new-API tags are forbidden.
