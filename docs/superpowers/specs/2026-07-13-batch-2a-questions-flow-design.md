...
Instrumentation is observational only. The temporary patched `App` may expose a test-only, read-only observer hook that receives already-computed component/render state after React actions settle. It may extract an existing pure calculation into a helper only when the component itself is changed to call that exact helper. The patch and extractor must not implement, duplicate, infer, or call test-only substitutes for branch rules, repairs, validation, answer application, navigation, forced resolution, completion, or disabled-option policy. It must not call exported legacy helpers to manufacture expected trace values. Every frozen frame comes from the actual rendered component and its actual state after an actual public action has settled; a single public action may produce multiple observed frames when the existing component performs a forced skip or completion transition.

For maximum-selection observation, instrumentation reads the disabled state actually exposed by the rendered legacy component or DOM. `disabledOptionIds` contains only currently visible disabled options, in legacy display order; hidden options are forbidden. The current legacy contract leaves selected ordinary options and exclusive options enabled; instrumentation still records actual rendered state rather than inferring either classification. Instrumentation must not invoke a disabled option's click handler or synthesize an option action. The authoring seed reaches the cap through real public selections and records the disabled third ordinary option in the settled frame; it contains no extra `select` action for that disabled option. An observation outside this contract requires an explicit contract/model revision; instrumentation must not reinterpret it or add an adapter exception.
...
interface LegacyObservableTraceFrame {
  ...
  readonly disabledOptionIds?: readonly OptionId[]
...
- `select` represents one actual enabled-option click, requires the option to be absent in the directly observed pre-action selection and present in the settled post-action selection, and emits one settled `toggle` frame.
- `deselect` represents one actual selected-option click, requires the option to be present before and absent after, and emits one settled `toggle` frame.
- An unchanged toggle rejects the case. It is not converted into a successful public action or a maximum-selection behavior.
...
Frames record the current displayed question and its visible option IDs, the currently visible options the legacy UI directly exposes as disabled, the component's actual pending selection, the actual legacy answer state used or saved by the component, actual forced auto-answer/skip transitions, the question or results screen actually reached, the results completion marker, and actual visible-option or answer changes after branch-driving actions. `disabledOptionIds` is an ordered subset of the same frame's `visibleOptionIds`; hidden options are forbidden, and disabled state is never inferred from the selection count or exclusive policy. Arrays and object keys are normalized only for stable serialization when that normalization does not change the observed legacy value or display order.
...
| Every legacy-representable displayed question, visible and disabled option state, public action, observed forced skip, navigation target, completion marker, and branch-visible/answer change | Frozen observable traces |
...
Trace coverage tags are limited to facts mechanically derivable from validated actions and observable frames: displayed question IDs, visible and disabled option IDs scoped to their question, action types, transition types, forced-skip, navigation targets, completion, validated pending-selection effects, `behavior:max-selection-blocked`, and observed branch changes. `deriveObservableCoverage({ actions, frames })` applies one identical predicate to frozen and received traces after strict validation: for a real successful `select`, compare its settled `toggle` frame with the immediately preceding settled frame and find at least one option ID that is visible in both frames, absent from the preceding frame's `disabledOptionIds`, present in the toggle frame's `disabledOptionIds`, and absent from the toggle frame's `pendingOptionIds`. The selected action option must satisfy the independently validated absent-before/present-after rule, so it cannot be that still-visible, newly disabled, unselected option; the one-action/one-toggle contract emits no action for the disabled option. This predicate uses no option classification, `maxSelections`, compiled policy, or expected fixture data. `behavior:max-no-op` is removed from observable parity. Seeds never contain coverage tags; visible/disabled options, navigation targets, forced transitions, completion, and named behavior outcomes therefore cannot become hand-authored seed expectations. Semantic-class, repair, diagnostic, invalid-input, dependency, reachability, and new-API tags are forbidden.
...
New-runtime parity is a projection, not an internal-model comparison. A dedicated runtime adapter starts from a fresh state, performs the same ordered legacy-representable public actions, lets each transition settle, and emits only `LegacyObservableTraceFrame` fields that the legacy trace schema permits. `select`/`deselect` update only adapter pending selection and must produce the exact absent-before/selected-after or selected-before/absent-after change; an unchanged result rejects replay. Their legacy-shaped observable answer field projects that changed pending value so it matches the legacy component's immediate answer change, while `AnswerDraft` remains unchanged. Each projected interactive frame computes ordered `disabledOptionIds` without a DOM from the compiled visible option IDs, current canonical pending IDs, compiled exclusive option IDs, and compiled `maxSelections`: a visible option is disabled exactly when it is not selected, is not exclusive, the pending-selection count is at least `maxSelections`, and no exclusive option is selected. Filtering the compiled visible IDs in their display order preserves ordering; selected and exclusive options remain enabled. The adapter must not consult frozen expected data or add a legacy exception. Any observed difference is a parity mismatch that requires an explicit contract/model revision. `continue` calls `applyAnswer` with the current pending selection, evaluates the accepted draft to settled flow state, emits one `submit`, emits zero or more actual compiled forced transitions as `forced-skip`, and then emits exactly one `next` or `complete`, all under the same action index. A rejected `applyAnswer` means the seed is not legacy-representable and rejects fixture authoring/replay; rejected API metadata is never frozen. `previous` performs only previous navigation, does not call `applyAnswer`, and preserves adapter pending projections so the observable legacy-shaped answer view still reflects prior option clicks while the formal draft remains unchanged. The adapter may use runtime APIs to execute actions, but it must not serialize `FlowState`, `ApplyAnswerResult`, compiler metadata, or any excluded field into received parity data. Stable projected frames are then compared to the frozen trace in order.
